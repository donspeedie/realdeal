...
function calculateStrategy(method, prop, params, pricePerSqFt, twoBedAvg, bedroomAnalysis = []) {
  try {
    ...
    if (method === "Rental") {
      const rentEstimate = Number(params.monthlyRent) ||
        Number(prop.rentZestimate) ||
        estimateMonthlyRent(futureValue);
      ...
      const resaleValue = futureResaleValue; // Use appreciated value after 5 years
      ...
      // For Rental: Only include upfront and selling costs, NOT ongoing expenses
      // (loanPayments, propertyTaxes, propertyIns are already deducted in annualCashFlow)
      const totalCosts = calculateTotalCosts({
        purchasePrice,
        impValue,
        loanPayments: 0, // Already deducted from annualCashFlow
        loanFees: loanDetails.loanFees,
        permitsFees: config.permitsFees,
        propertyTaxes: 0, // Already deducted from NOI
        propertyIns: 0, // Already deducted from NOI
      ...
      const totalReturn = Number(resaleValue) + Number(netRentalIncomeFiveYears) - Number(purchasePrice) - Number(impValue) - Number(config.permitsFees);
      const totalCapitalInvested = Number(loanDetails.downPayment) + Number(impValue) + Number(config.permitsFees);
      const groc = totalCapitalInvested > 0 ? totalReturn / totalCapitalInvested : 0;
      ...
      return enhanceStrategyWithIRR({
        method,
        futureValue: Number(resaleValue) || 0, // Appreciated property value after 5 years (ARV)
        totalReturns: (Number(resaleValue) + Number(netRentalIncomeFiveYears)) || 0, // Total returns: ARV + rental income
        futureLivingArea: Number(futureLivingArea) || 0,
        impValue: Number(impValue) || 0,
        loanAmount: round(Number(loanDetails.loanAmount) || 0),
        downPayment: round(Number(loanDetails.downPayment) || 0),
        monthlyPayment: Number(loanDetails.monthlyPayment) || 0,
        loanPayments: Number(loanDetails.loanPayments) || 0,
        loanFees: Number(loanDetails.loanFees) || 0,
        permitsFees: Number(prop?.permitsFees || config.permitsFees) || 0,
        propertyTaxes: Number(config.propertyTaxes) || 0,
        propertyIns: Number(config.propertyIns) || 0,
        propTaxIns: Number(prop?.propTaxIns || (Number(config.propertyTaxes || 0) + Number(config.propertyIns || 0))),
        sellingCosts: Number(sellingCosts) || 0,
        totalCosts: round(Number(totalCosts) || 0),
        netSaleProceeds: netSaleProceeds,
        netReturn: netReturn,
        holdingPeriodYears: holdingPeriodYears,
        appreciationRate: appreciationRate,
        futureResaleValue: futureResaleValue,
        sellingCostsAtResale: sellingCostsAtResale,
        resaleValue: resaleValue,
        estimatedRentalIncome: netRentalIncomeFiveYears,
        rentEstimate: round(Number(rentEstimate) || 0),
        monthlyRent: round(Number(monthlyRent) || 0),
        annualRent: round(Number(annualRent) || 0),
        annualNOI: round(Number(annualNOI) || 0),
        annualCashFlow: round(Number(annualCashFlow) || 0),
        monthlyCashFlow: round(Number(monthlyCashFlow) || 0),
        expenses: {
          annualMaintenance: round(Number(annualMaintenance) || 0),
          annualOperatingExpenses: round(Number(annualOperatingExpenses) || 0),
          annualPropertyManagement: round(Number(annualPropertyManagement) || 0),
          annualUtilities: round(Number(annualUtilities) || 0),
          annualVacancy: round(Number(annualVacancy) || 0),
          totalAnnualExpenses: round(Number(annualMaintenance + annualOperatingExpenses +
            annualPropertyManagement + annualUtilities + annualVacancy +
            config.propertyTaxes + config.propertyIns + loanDetails.loanPayments) || 0),
        },
        rates: {
          maintenanceRate: Number(config.maintenanceRate) || 0,
          operatingExpenseRate: Number(config.operatingExpenseRate) || 0,
          propertyManagementFeeRate: Number(config.propertyManagementFeeRate) || 0,
          vacancyRate: Number(config.vacancyRate) || 0,
        },
        cashNeeded: round(Number(cashNeeded)) || 0,
        roe: Number(roe) || 0,
        groc: Number(groc) || 0,
        dscr: Number(dscr) || 0,
        purchCloseDate: closingDates.purchase,
        valuationMethod: config.valuationMethod || "rental_income_approach",
        extraValue: round(Number(config.extraValue) || 0),
        mtgRate: Number(config.mtgRate) || 0,
    ...
    // GROC Calculation:
    const totalReturn = Number(futureValue) - Number(purchasePrice) - Number(impValue) - Number(config.permitsFees);
    const totalCapitalInvested = Number(loanDetails.downPayment) + Number(impValue) + Number(config.permitsFees);
    const groc = totalCapitalInvested > 0 ? totalReturn / totalCapitalInvested : 0;

    return enhanceStrategyWithIRR({
      method,
      futureValue: Number(futureValue) || 0,
      totalReturns: Number(futureValue) || 0, // For non-rental strategies, totalReturns = futureValue
      futureLivingArea: Number(futureLivingArea) || 0,
      impValue: Number(impValue) || 0,
      loanAmount: round(Number(loanDetails.loanAmount) || 0),
      downPayment: round(Number(loanDetails.downPayment) || 0),
      ...
      sellingCosts: Number(sellingCosts) || 0,
      totalCosts: round(Number(totalCosts) || 0),
      netSaleProceeds: round(Number(netSaleProceeds) || 0),
      netReturn: round(Number(netReturn) || 0),
      netROI: Number(netROI) || 0,
      cashNeeded: round(Number(cashNeeded)) || 0,
      cashOnCashReturn: Number(cashOnCashReturn) || 0,
      roe: Number(roe) || 0,
      groc: Number(groc) || 0,
      dscr: 0,
      purchCloseDate: closingDates.purchase,
      ...
      totalValue: Number(loanDetails.totalProjectCost) || 0,
      mortgage: Number(loanDetails.mortgage) || 0,
      optimalOffer: round(Number(optimalOffer) || 0),
      avgDollarPerSqft: futureLivingArea > 0 ? round(Number(futureValue) / Number(futureLivingArea)) : 0,
      avgDollarPerBdrm: bedrooms > 0 ? round(Number(futureValue) / Number(bedrooms)) : 0,
      avgRentPerSqft: (prop.rentZestimate && futureLivingArea > 0) ? round(Number(prop.rentZestimate) / Number(futureLivingArea)) : 0,
    }, params, config.duration);
  } catch (error) {
...
function estimateMonthlyRent(propertyValue) {
  ...
  // More generous rent estimation (1.2% rule for better rental opportunities)
  return round(safeValue * 0.012); // Increased from 1% to 1.2% of property value
}
...
function estimateFutureValue(method, futureArea, pricePerSqFt, config, bedrooms, bedroomAnalysis, filteredComps, subjectProperty) {
  if (method === "Add-On") {
    ...
    const aduIncomeValue = calculateADUIncomeValue(subjectProperty, filteredComps, config);
...
function calculateADUIncomeValue(subjectProperty, filteredComps, config) {
  ...
  if (subjectProperty.rentZestimate && subjectProperty.bedrooms === 2) {
    ...
    if (twoBedComps.length > 0) {
      ...
      // Estimate rent using 1% rule as fallback
      twoBedRentZestimate = Math.round(avgTwoBedPrice * 0.01);
    }
  }
